            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        ...
                        <h6>{{__('appfiy::messages.layoutTypeList')}}</h6>
                        ...
                        <form method="post" role="form" id="search-form">
                            <table id="leave_settings" class="table table-bordered datatable table-responsive mainTable text-center">
                                ...
                                <tr>
                                    ...
                                    <th>{{__('appfiy::messages.inputType')}}</th>
                                ...
                                @if(isset($layoutTypeProperties) && count($layoutTypeProperties)>0)
                                    <tbody>
                                        @php $i=1; @endphp
                                        @foreach($layoutTypeProperties as $layoutTypePro)
                                            <tr>
                                                <td>{{$i}}</td>
                                                <td>{{$layoutTypePro->name}}</td>
                                                <td>{{$layoutTypePro->input_type}}</td>
                                                <td>{{$layoutTypePro->value}}</td>
                                                <td>{{$layoutTypePro->default_value}}</td>
                                                <td>
                            ...
                            @if(isset($layoutTypeProperties) && count($layoutTypeProperties)>0)
                                <div class=" justify-content-right">
                                    {{ $layoutTypeProperties->links('layouts.pagination') }}
                                </div>
                            @endif
                        </form>
                    </div>
